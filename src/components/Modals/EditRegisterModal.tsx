...
export default function EditRegisterModal({isOpen, requestClose, IdOfRegister} : EditRegisterModalProps){
    ...
    const [ retornoDasEmpresa , setRetornoDasEmpresas ] = useState([])
    ...
    const empresas = retornoDasEmpresa

    const [ retornoDoLancamento, setRetornoDoLancamento ] = useState([])

    const [InputData, setInputData] = useState(new Date()) //setar variaveis no use state aaaa-mm-dd
    ...
    // const chamada = await api.get(`Schedule/${Id}/07_2022`).then(response => setRetornoDoLancamento(response.data)).catch(e => console.log(e))
    // }

    // recuperaDados()

    // useEffect(() => {
    //     api.get(`Schedule/${Id}/07_2022`).then(response => 
    //     {
    //         setRetornoDoLancamento(response.data)
